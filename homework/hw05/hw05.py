...
def replace_leaf(t, old, new):
    ...
    for branch in branches(t):
        new_branches += [replace_leaf(branch, old, new)]
...
def balanced(m):
    ...
    # for item1 in end(left(m)):
    #     if is_mobile(item1):
    #         if not balanced(item1):
    #             return False
    # for item2 in end(right(m)):
    #     if is_mobile(item2):
    #         if not balanced(item2):
    #             return False
...
def make_withdraw(balance, password):
    ...
    """
    wrong_password = []
    def withdraw(amount, pw_atempt):
        nonlocal balance, wrong_password
        if len(wrong_password) == 3:
            return "Your account is locked. Attempts: ['{0}', '{1}', '{2}']".format(wrong_password[0],wrong_password[1],wrong_password[2])
        if pw_atempt != password:
            wrong_password += [pw_atempt]
...
def make_joint(withdraw, old_password, new_password):
    ...
    if type(a) != str:  #important: if statement is evaluted to
        return withdraw2
...
def generate_paths(t, x):
    ...
    if label(t) == x:
        yield [label(t)]
    for branch in branches(t):
        for item in generate_paths(branch,x):
            yield [label(t)] + item
###################
# Extra Questions #
###################
