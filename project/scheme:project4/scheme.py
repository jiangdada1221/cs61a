...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    >>> scheme_eval(expr2, create_global_frame())
    12
    """
    # Evaluate atoms
...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        # except Exception:
        #     try:
        #         return self.parent.bingdings[symbol]
        #     except Exception:
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            return self.fn(*python_args)    #将list中的元素取出
        except TypeError:
            raise SchemeError
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    >>> expr = read_line('(a (+ 2 3))')
    >>> do_define_form(expr,create_global_frame())
    'a'
    >>> 'a'
    5
    """
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        b = scheme_eval(expressions.second.first,env)
        env.define(target,b)
        ...
        env.define(target.first,LambdaProcedure(formal,body,env))
        return target.first
        # END PROBLEM 10
    else:
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        return do_cond_form(expressions,env)
...
def make_let_frame(bindings, env):
    ...
    check_form(bindings.first.second,1)
    formals,vals = nil,nil
    while bindings != nil:
        check_formals(bindings.first.first)

        formals, vals = Pair(bindings.first.first,formals), Pair(scheme_eval(bindings.first.second.first,env),vals)
...
def do_define_macro(expressions, env):
    ...
    check_form(target,0)
    macro , formals = target.first, target.second
    body = expressions.second
    env.defien(macro, MacroProcedure(formals, body, env))
    return macro
    # END Problem 21
