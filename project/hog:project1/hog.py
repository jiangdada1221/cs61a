...
def is_swap(player_score, opponent_score):
    ...
    """
    if player_score >= 100:
        i = abs((player_score // 10) % 10 - player_score % 10)
    else:
        i = abs((player_score // 10)  - player_score % 10)
    if opponent_score >= 100:
        j = abs((opponent_score // 10) % 10 - opponent_score % 10)
    else:
        j = abs((opponent_score // 10) - opponent_score % 10)
    return i == j
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def say(score0,score1):
        if who == 0:
            a = score0 - previous_score
            if(a > previous_high):
                print(a,'point(s)! That\'s the biggest gain yet for Player',who)
                return announce_highest(who,a,score0)
            else:
                return announce_highest(who,previous_high,score0)
        if who == 1:
            a = score1 - previous_score
            if(a > previous_high):
                print(a,'point(s)! That\'s the biggest gain yet for Player',who)
                return announce_highest(who,a,score1)
            else:
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    k = 1
    while i <= 10:
        b = make_averaged(roll_dice,num_samples)(i,dice)
        if b > a :
            a = b
            k = i
        i = i + 1
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if score + free_bacon(opponent_score) > 100:
        a = score + free_bacon(opponent_score)
        if is_swap(a,opponent_score):
            b = GOAL_SCORE - score
            if b > 12:
                return 3
            elif b > 6:
                ...
                return 1
    a = score - opponent_score

    if a >= 0:
        return swap_strategy(score,opponent_score,margin=2,num_rolls=1)
    if a <= -60:
        return swap_strategy(score,opponent_score,margin=20,num_rolls=10)
    if a <= -40:
        return swap_strategy(score,opponent_score,margin=20,num_rolls=8)
    if a <= -25:
        return swap_strategy(score,opponent_score,margin=13,num_rolls=6)
    if a <= -10:
        return swap_strategy(score,opponent_score,margin=8,num_rolls=3)
    if a <= 0:
        return swap_strategy(score,opponent_score,margin=3,num_rolls=1)
##########################
