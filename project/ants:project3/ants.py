            ...
            if self.ant is None:
                ...
                if self.ant.is_container:
                    ...
                    if insect.is_container:
                        ...
                        self.ant = insect
                    # else: assert self.ant is None, 'Two ants in {0}'.format(self)
                    else: raise AssertionError('Two ants in {0}'.format(self))
                # END Problem 9
        else:
        ...
        if insect.is_ant:
            ...
            # Special handling for container ants
                ...
                if hasattr(self.ant, 'is_container') and self.ant.is_container \
                    ...
                    if isinstance(self.ant.contained_ant, QueenAnt):
                        if self.ant.contained_ant.mark > 1:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        """
        if isinstance(self,ShortThrower) or isinstance(self, LongThrower):
            min, max, range= self.min_range, self.max_range, 0
        else:
            min,max,range = 0,float('inf'),0
        if self.place.bees and min == 0:
            return random_or_none(self.place.bees)
        else:
            refer = self.place.entrance

            while refer != hive:
                range += 1
                if refer.bees and range >= min and range <= max:
                    return random_or_none(refer.bees)
                refer = refer.entrance
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.mark > 1:
            ...
            refer = self.place.exit
            while refer != None:
                if refer.ant != None:
                    if refer.ant.is_container == True:
                        if refer.ant.contained_ant and (refer.ant.contained_ant.is_buffed == False):
                            refer.ant.contained_ant.damage *= 2
                            refer.ant.contained_ant.is_buffed = True
                        if isinstance(refer.ant, TankAnt) and (refer.ant.is_buffed == False):
                            refer.ant.damage *= 2
                            refer.ant.is_buffed = True

                    else:
                        if not refer.ant.is_buffed:
                            refer.ant.damage *= 2
                            refer.ant.is_buffed = True
                refer = refer.exit
