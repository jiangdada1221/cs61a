...
def replace_elem(lst, index, elem):
    ...
    return [lst[x] for x in range(index)] + [elem] + [lst[x] for x in range(index+1,len(lst))]
...
def put_piece(board, max_rows, column, player):
    ...
    """
    i = max_rows
    while(get_piece(board,i-1,column) != '-'):
        i = i - 1
        if i == 0:
            index = -1
            return index,board
    index = i - 1
    newboard = replace_elem(board,index,replace_elem(board[index],column,player))
    return index,newboard
...
def make_move(board, max_rows, max_cols, col, player):
    ...
    if(col >= max_cols or col < 0):
        return -1,board
    while(get_piece(board,i-1,col) != '-'):
        i = i - 1
        if i == 0:
            index = -1
            return index,board
    index = i - 1
    newboard = replace_elem(board,index,replace_elem(board[index],col,player))
    return index,newboard
