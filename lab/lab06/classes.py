...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        # for card2 in opponent.deck.cards:
        #     if other_card.attack == card.attack or other_card.defense == card.defense:
        #
        #         opponent.deck.cards.remove(card2)
        # important!: this will change deck.cards and change this iteration
        # k,i = 0,len(opponent.deck.cards)
        # print(opponent.deck.cards[1])
        # while k < i:
        #     if other_card.attack == opponent.deck.cards[k].attack or other_card.defense == opponent.deck.cards[k].defense:
        #         opponent.deck.cards.remove(opponent.deck.cards[k])
        #     k += 1
        a ,k= len(opponent.deck.cards),0

        while k < a:
            if other_card.attack == opponent.deck.cards[k].attack or other_card.defense == opponent.deck.cards[k].defense:

                opponent.deck.cards.pop(k)
                a = a - 1
                k = k - 1         #pop 后得改变长度
            k += 1
